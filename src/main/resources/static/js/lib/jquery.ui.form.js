...
$.widget( "ui.validate", {
	options : {
		...
		validators : {
			...
			password: {
				message : "Password length needs to be 8 to 16 digits and must contain at least two of letters, numbers and symbols",
				validate : function(value,element) {
					if(value == null || value.length == 0){
						return true;
					}
					...
					if(count<2 || value.length<8 || value.length>16){
						return false;
					}else{
						return true;
					}
